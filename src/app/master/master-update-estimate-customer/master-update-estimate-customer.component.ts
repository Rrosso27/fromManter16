...
@Component({
  ...
  styleUrls: [
    "./master-update-estimate-customer.component.scss",
...
export class MasterUpdateEstimateCustomerComponent implements OnInit {
  ...
  getPendingForklift() {
    this.resumeneServices
      .getForkliftPendingGeneral(this.selectedForkliftId)
      .then((data) => {
        const resp: any = data;
        console.log(
  ...
  getEstimateSpecific(id: number) {
    this.estimateService
      ...
      .then((data) => {
        ...
        console.log(
          "cotizacion actual " + JSON.stringify(this.currentEstimate)
        );
  ...
  getEstimateParts() {
    if (this.estimateId) {
      this.estimateService
        ...
        .then((data) => {
          console.log(
            "importante información del estimate:" + JSON.stringify(data)
          );
          console.log(data);
          const resp: any = data;
          ...
          if (resp.data.length > 0) {
            ...
            console.log("_--------------------")
            console.log(this.rowsItemsparts )
            console.log("_--------------------")
  ...
  getForklifs() {
    console.log(
      "este es el valor de customerId:" + this.selectedBranchOfficeId
    );
  ...
  updateItems(item: any) {
    ...
    console.log("item.iva = "+ item.iva )
    this.priceIva = item.iva;
    console.log("priceIva = "+ this.priceIva );

    this.preceIvaTol = item.total_iva ;
  ...
  updateWorkForceItems(item: any) {
    ...
    this.workforceSubtotalUpdate = this.finalFormatStandard(
      Number(item.subtotal).toFixed(0)
    );
 ...
 */
  ...
  showShippingCountriesDhl(
    ...
    for (let item of this.showShippingCountriesDhlFilter) {
      ...
      if (
        ...
        this.extraPercentage =  Number(item.extra_percentage) / 100;
      ...
      if (this.priceList == 5 || this.priceList == 6) {
        ...
        if (this.priceList == 5) {
          this.suggestedPrice = Number(
            Number(this.unitCost / this.thirdService)
          ).toFixed(0);
          ...
          this.suggestedPrice = Number(
            Number(this.unitCost / this.nationalService)
          ).toFixed(0);
          this.price = this.suggestedPrice;
          this.subtotal = this.suggestedPrice * this.quantity;
          this.lowPrice = this.suggestedPrice;
  ...
  finalOperation(country: number) {
    ...
    let increment = Number(priceQuantity) * Number(this.extraPercentage);

    let total = Number(priceQuantity) + Number(increment);
    this.subtotal = total;
  ...
  calcularIva() {
    let priceIva = this.price * this.iva;
    this.priceIva1 =  this.finalFormat (Number(priceIva).toFixed(0));
    this.preceIvaTol1 = this.finalFormat( Number(this.subtotal + priceIva).toFixed(0)) ;
  }
  ...
  finalOperationUpdate(country: number) {
    ...
    let priceQuantity = (
      Number(this.priceUpdate) * Number(this.quantityUpdate)
    ).toFixed(0);
    let increment = Number(priceQuantity) * Number(this.extraPercentage);
    let total = Number(priceQuantity) + Number(increment);
    this.subtotalUpdate = total.toFixed(0);
  ...
  calcularIvaUpdate() {
    let priceIvaUdate = this.priceUpdate * this.iva;
    this.priceIva = Number(priceIvaUdate).toFixed(0);
    this.preceIvaTol = Number(this.subtotalUpdate + priceIvaUdate).toFixed(0);
  }
  ...
  uploadAll() {
    ...
    if (
      this.selectedFilesImages.length == 0 &&
  ...
  uploadFiles() {
    for (let fileCurrent of this.selectedFiles) {
      ...
      let nameTemp = this.removeAccents(
        this.consecutive + this.normalize(file.name.replace(/\s/g, ""))
      );
  ...
  uploadFilesImages() {
    for (let fileCurrent of this.selectedFilesImages) {
      ...
      console.log(nameTemp);
      this.uploadService
        .uploadFilesAll(file, this.estimateId, 1, nameTemp)
        .then((res) => {
          console.log("s3info" + JSON.stringify(res));
          this.s3info = res;
          swal.close();
    ...
    if (item.id) {
      swal({
        ...
        if (willDelete.value) {
          ...
          swal.showLoading();
          this.estimateService
            .deleteEstimateFile(Number(item.id))
            .then((data) => {
              swal.showLoading();
              const resp: any = data;
              console.log(resp);
  ...
  /* validateEmail( email )
 {
  ...
  createEstimateDetailWorkforce() {
    ...
    console.log(
      "Pureba llamada a metodo; mandar a servicio, subtotal es: " + subtotalTemp
    );
  ...
  calculateSubtotalWorkUpdate() {
    ...
    console.log(
      "this.workforceSubtotalUpdate ----- " + this.workforceSubtotalUpdate
    );
  ...
  updateEstimate() {
    ...
    if (this.selectedBusinessId) {
      console.log(
        "entroooooooooooooooooooooooooo pssssssssssssssssssssssssssssssss siiiiiiiiiiiii"
      );
  ...
  calculateSubtotalUpdate() {
    ...
    priceUpdateTemp = priceUpdateTemp
      .toString()
      .replace(".", "")
      .replace(".", "")
      .replace(".", "")
      .replace(".", "")
      .replace(",", "."); //this.finalFormatStandard(priceUpdateTemp);
  ...
  getFilesEstimate() {
    console.log("ingreso que paso ps");
    this.estimateService
      .getEstimateDetailFiles(Number(this.estimateId))
      .then((data) => {
        const resp: any = data;
        if (resp.data) {
          console.log("-----------------------");
